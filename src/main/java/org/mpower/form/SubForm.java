...
public class SubForm {
	...
	public void buildSubFormInstanceFields() {
		...
		System.out.println(
		    "building forminstance for subform- " + this.name + " with deafault bind path- " + subFormDefaultBindPath);
		...
		try {
			...
			for (int i = 0; i < nodeList.getLength(); i++) {
				...
				System.out
				        .println(nodeList.item(i).getNodeName() + " -has number of children- " + childNodeList.getLength());
				for (int j = 0; j < childNodeList.getLength(); j++) {
					...
					if (currentNode.getNodeType() == Node.ELEMENT_NODE && SubmissionBuilder.variableMapperForForm
					        .get(subFormDefaultBindPath + "/" + currentNode.getNodeName()) != null) {
						if (SubmissionBuilder.variableMapperForForm
						        .get(subFormDefaultBindPath + "/" + currentNode.getNodeName())
						        .equalsIgnoreCase("groupTag")) {
							...
							for (int k = 0; k < groupTagChildNodeList.getLength(); k++) {
								...
								String path = subFormDefaultBindPath + "/" + currentNode.getNodeName() + "/"
								        + innerNode.getNodeName();
								
								if (innerNode.getNodeType() == Node.ELEMENT_NODE
								        && SubmissionBuilder.variableMapperForForm.get(path) != null) {
									System.out.println(
									    path + " --*****-- " + SubmissionBuilder.variableMapperForForm.get(path));
									hm.put(SubmissionBuilder.variableMapperForForm.get(path),
									    innerNode.getTextContent().trim());
								}
							}
						} else
							hm.put(SubmissionBuilder.variableMapperForForm.get(
							    subFormDefaultBindPath + "/" + currentNode.getNodeName()), currentNode.getTextContent());
					}
