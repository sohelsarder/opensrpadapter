...
public class Form {
	...
	public void buildFields() {
		...
		for (Field field : fields) {
			...
			if (field.bind != null) {
				...
				boolean convertibleStartAndEndDateTimeField = field.name.equalsIgnoreCase("start")
				        || field.name.equalsIgnoreCase("end");
				...
				boolean convertibleBirthNotificationDateTimeField = checkIfConvertibleBirthNotificationDateTimeField(
				    field.name);
				
				if (convertibleBirthNotificationDateTimeField) {
					String convertibleDateTimeStr = field.value;
					String adaptedDateStr = this.convertBirthNotificationDateTimeFieldValue(convertibleDateTimeStr);
					field.value = adaptedDateStr;
				}
				...
				if (SubmissionBuilder.variableMapperForForm.get("entityID").startsWith("/")) {
					...
					SubmissionBuilder.variableMapperForForm.put("entityID", entityId);
				} else {
	...
	private static String extractLastNode(String bindNode) {
		...
		String retVal = bindNode.substring(lastIndexOfSlash + 1, bindNode.length());
		return retVal;
	}
